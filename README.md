...
## Linux Build
NOTE: still under development- don't expect absolute stability.
...
Note: if you get an error in compiling due to `-lcurl-gnutls` being missing, you need a symlink: `sudo ln -s /usr/lib32/libcurl-gnutls.so.4 /usr/lib32/libcurl-gnutls.so`


Instructions:

0) Make sure the path leading up the `team-comtress-2` has no spaces in it. This will lead to errors compiling Protobuf and gperftools (needed for tcmalloc).
1) Prepare for unforseen consequences (mainly it crashing during compilation.)
2) Run `./build.sh`
2.5) To make sure it compiled correctly, run it again. If you see no errors, then it compiled. Success!
3) Install your TF2 assets into `../game/tf`. If you have TF2 and steam installed on the same drive, running `cp -rn ~/.steam/steam/steamapps/common/Team\ Fortress\ 2/* ./` will work.
4) Run in the main repo folder `./run.sh`.
5) If you get some errors about a missing client library, enter the `bin` folder, which is in the `game` folder you're currently in and run this command: `ln -s ../tf/bin/* .`, then try running again. If it still doesn't work, try rebooting.
